...
func handleMethod(method string, rawRequest []byte) ([]byte, error) {
	...
	case "model.route":
		return route(rawRequest)
	case "request.intercept_before":
		return passThrough(rawRequest)
	case "request.intercept_after":
		return interceptAfter(rawRequest)
	default:
...
func registration() map[string]any {
	return map[string]any{
		...
		"metadata": map[string]any{
			"Name":             "antigravity-tiered-models",
			"Version":          "0.1.0",
			"Author":           "local",
...
func route(rawRequest []byte) ([]byte, error) {
	...
	return okEnvelope(map[string]any{
		...
		"TargetKind": "provider",
		"Target":     provider,
		"TargetModel": modelTiered,
		"Reason":     "map Gemini 3.7 Flash thinking aliases to Antigravity tiered",
	})
}

func passThrough(rawRequest []byte) ([]byte, error) {
...
func interceptAfter(rawRequest []byte) ([]byte, error) {
	var req interceptRequest
	if len(rawRequest) > 0 {
		if err := json.Unmarshal(rawRequest, &req); err != nil {
			return nil, err
	...
	level := aliasLevel(req.RequestedModel)
	if level == "" || !isAntigravity(req.ToFormat) {
		return okEnvelope(map[string]any{"Headers": req.Headers, "Body": req.Body})
	}
...
func isAlias(model string) bool {
	...
	case modelLow, modelMedium, modelHigh, modelTiered:
		return true
	default:
		return false
	}
...
func aliasLevel(model string) string {
	switch strings.ToLower(strings.TrimSpace(model)) {
	case modelLow:
		return "low"
	case modelMedium:
...
func isAntigravity(format string) bool {
	format = strings.ToLower(strings.TrimSpace(format))
	return format == "" || strings.Contains(format, "antigravity")
}

// rewriteThinkingBody updates the provider payload only. It intentionally leaves
// non-JSON/canonical payloads untouched so the plugin cannot corrupt another route.
